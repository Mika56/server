<template>
	<NcModal v-if="isModalOpen" id="unified-search" :name="t('core', 'Conversations')" :show.sync="isModalOpen"
		:size="'small'" :clear-view-delay="0" :title="t('core', 'Conversations')" @close="closeModal">
		<!-- Conversations -->
		<div class="unified-search-conversations-list-modal">
			...
			<div class="unified-search-conversations-list-modal__header">
				<NcTextField :value.sync="searchText" trailing-button-icon="close" class="search-form"
					:label="t('spreed', 'Search conversations')" :show-trailing-button="searchText !== ''"
			...
			<div class="unified-search-conversations-list-modal__body">
				<div id="room-list">
					<ul v-if="!loading && availableRooms.length > 0">
						<NcListItem v-for="room in availableRooms" :key="room.id" :name="room.displayName" :bold="false"
							:active="room.token === selectedRoom.token" :details="'1hr'"
							:counter-number="room.unreadMessages" counter-type="highlighted">
							<template #icon>
			...
			<div class="unified-search-conversations-list-modal__footer">
				<NcButton v-if="!loading && availableRooms.length > 0" type="primary" :disabled="!selectedRoom"
					@click="select">
...
export default {
	...
	methods: {
		...
		async fetchRooms() {
			...
			this.availableRooms = response.data.ocs.data.sort(this.sortConversations)
			console.debug('AVAILABLE ROOMS', this.availableRooms)
			this.loading = false
		},
		select() {
		...
		searchInConversation() {
			this.$emit('search-in:conversation', this.conversation)
			this.closeModal()
		},
	},
}
